...
class KeepAliveFrame extends RSocketFrame {
  ...
  KeepAliveFrame.fromBuffer(RSocketHeader header, RSocketByteBuffer buffer) {
    this.header = header;
    // RSocket spec: lastReceivedPosition is 64-bit (matches encodeKeepAlive).
    var lastReceivedPosition = buffer.readI64();
    if (lastReceivedPosition != null) {
      this.lastReceivedPosition = lastReceivedPosition;
    }
    // Remaining bytes (if any) are optional Keepalive Data; frameLength includes
    // the 6-byte type/flags header + 8-byte position (= 14) before payload.
    if (header.frameLength > 14) {
      payload = decodePayload(buffer, header.metaPresent, header.frameLength);
    }
