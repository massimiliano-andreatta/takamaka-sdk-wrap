...
abstract final class TkmChatProfileCrypto {
  ...
  /// Canonical Takamaka url64 for an Ed25519 identity / sign public key.
  ///
  /// Digests and peer cache keys may arrive with `.` vs `=` padding or
  /// whitespace; compare and store via this form when possible.
  static String canonicalizeIdentityKey(String value) {
    final trimmed = value.trim().replaceAll(RegExp(r'[\s\r\n]+'), '');
    if (trimmed.isEmpty) return trimmed;
    try {
      return TkmBase64Url.encode(TkmBase64Url.decodeFlexible(trimmed));
    } catch (_) {
      return trimmed.replaceAll(RegExp(r'[.=]+$'), '');
    }
  }

  /// Whether two identity / sign public keys refer to the same peer.
  static bool identityKeysMatch(String a, String b) {
    final left = a.trim();
    final right = b.trim();
    if (left.isEmpty || right.isEmpty) return false;
    if (left == right) return true;
    return canonicalizeIdentityKey(left) == canonicalizeIdentityKey(right);
  }
