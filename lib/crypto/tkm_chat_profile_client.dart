...
abstract final class TkmChatProfileClient {
  ...
  ///
  /// Parity with rsclient `ProfileClient.changedSince`: iterate **server digests**
  /// (not the request list), so a padding/encoding difference between the
  /// requested key and `digest.target` cannot skip a live profile.
  /// Prefer the caller-supplied string when it matches the digest target.
  static Future<List<String>> changedSince({
    ...
    for (var i = 0; i < unique.length; i += maxDigestBatch) {
      ...
      for (final digest in digests) {
        final hash = digest.blobHash.trim();
        if (hash.isEmpty) {
          // No profile / cleared / not visible — deliberately indistinguishable.
          continue;
        }
        final serverTarget = digest.target.trim();
        if (serverTarget.isEmpty) continue;

        // Prefer the form the caller asked for (UI / cache key continuity).
        final callerTarget = chunk.firstWhere(
          (t) => TkmChatProfileCrypto.identityKeysMatch(t, serverTarget),
          orElse: () => serverTarget,
        );
        final known = _knownHashForTarget(knownHashes, callerTarget) ??
            _knownHashForTarget(knownHashes, serverTarget);
        if (known == null ||
            known.isEmpty ||
            known.toLowerCase() != hash.toLowerCase()) {
          stale.add(callerTarget);
        }
      }
    }
    return stale;
  ...
  static String? _knownHashForTarget(
    Map<String, String> knownHashes,
    String target,
  ) {
    final direct = knownHashes[target];
    if (direct != null && direct.isNotEmpty) return direct;
    for (final entry in knownHashes.entries) {
      if (TkmChatProfileCrypto.identityKeysMatch(entry.key, target)) {
        return entry.value;
      }
    }
    return null;
  ...
  static Future<TkmPeerProfile> readPeerDetailed({
    ...
    try {
      ...
      // Owner published a blob but grant unwrap/unseal failed (re-reg / wrong enc).
      return const TkmPeerProfile(status: TkmPeerProfile.noGrant);
    }
  }
