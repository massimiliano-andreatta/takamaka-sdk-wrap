...
abstract final class TkmChatCrypto {
  ...
  static ({
    ...
    final targets = rawTargets is List
        ...
        : const <String>[];
    final reactionEmoji = action == 'reaction'
        ? reactionEmojiFromDecrypted(decrypted)
        : null;
    Map<String, dynamic>? fwContent;
  ...
  /// Reads reaction glyph from canonical `preview` (STD base64 of UTF-8) or
  /// the Phase-2 `emoji` field some peers send instead.
  static String? reactionEmojiFromDecrypted(Map<String, dynamic> decrypted) {
    ...
    final direct = (map['emoji'] as String?)?.trim();
    ...
    try {
      final bytes = TkmBase64Url.decodeFlexible(preview);
      final decoded = utf8.decode(bytes).trim();
      return decoded.isEmpty ? null : decoded;
    } catch (_) {
      try {
        final decoded = utf8.decode(base64Decode(preview)).trim();
        return decoded.isEmpty ? null : decoded;
      } catch (_) {
        return null;
      }
