...
void main() {
  ...
  group('grant paging arithmetic', () {
    ...
    test('canonicalizeIdentityKey equates . and = padding', () {
      const withDot = '919JbL1nNrIGWaKLYasmnoyl4kTOqYyVNXo7Pt2y1c8.';
      final withEq = withDot.replaceAll('.', '=');
      expect(
        TkmChatProfileCrypto.canonicalizeIdentityKey(withDot),
        TkmChatProfileCrypto.canonicalizeIdentityKey(withEq),
      );
      expect(
        TkmChatProfileCrypto.identityKeysMatch(withDot, withEq),
        isTrue,
      );
      expect(
        TkmChatProfileCrypto.identityKeysMatch(withDot, 'other'),
        isFalse,
      );
    });
  });
